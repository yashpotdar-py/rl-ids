...
class IDSPlotter:
    ...
    def plot_training_metrics(
        self, metrics_path: Union[str, Path], save_name: str = "training_metrics_overview"
    ) -> None:
        ...
        ax1.plot(episodes, accuracy, color=self.colors["primary"], linewidth=2, alpha=0.8)
        ax1.fill_between(episodes, accuracy, alpha=0.3, color=self.colors["primary"])
        ax1.axhline(
            y=accuracy.mean(),
        ...
        ax2.plot(episodes, reward, color=self.colors["secondary"], linewidth=2, alpha=0.8)
        ax2.fill_between(episodes, reward, alpha=0.3, color=self.colors["secondary"])
        ...
        if window > 1:
            ...
            ax2.plot(
                episodes, reward_ma, color=self.colors["dark"], linewidth=3, label=f"MA({window})"
            )
        ...
        ax2.set_title("Training Reward Progress", fontsize=14, fontweight="bold")
        ...
        loss = df["Train_Loss"]
        ax3.plot(episodes, loss, color=self.colors["accent"], linewidth=2, alpha=0.8)
        ax3.fill_between(episodes, loss, alpha=0.3, color=self.colors["accent"])

        # Add moving average for loss
        if window > 1:
            loss_ma = loss.rolling(window=window, center=True).mean()
            ax3.plot(
                episodes, loss_ma, color=self.colors["dark"], linewidth=3, label=f"MA({window})"
            )
        ...
        ax5.hist(
            episode_lengths, bins=30, alpha=0.7, color=self.colors["primary"], edgecolor="black"
        )
        ax5.axvline(
        ...
        if "Val_Accuracy" in df.columns:
            ...
            ax6.fill_between(
                episodes, train_acc_resampled, val_acc, alpha=0.2, color=self.colors["accent"]
            )
            ax6.set_title("Training vs Validation Accuracy", fontsize=14, fontweight="bold")
        ...
        ax7.plot(episodes, accuracy, alpha=0.3, color=self.colors["primary"], label="Raw Accuracy")
        ax7.plot(
            episodes,
            rolling_mean,
            color=self.colors["primary"],
        ...
        ax7.set_title("Training Stability Analysis", fontsize=14, fontweight="bold")
        ...
        final_accuracy = accuracy.iloc[-20:].mean() if len(accuracy) >= 20 else accuracy.mean()
        max_accuracy = accuracy.max()
        accuracy_std = accuracy.std()
        final_reward = reward.iloc[-20:].mean() if len(reward) >= 20 else reward.mean()

        summary_data = [
            ["Metric", "Value", "Description"],
            ["Final Accuracy (last 20)", f"{final_accuracy:.4f}", "Average of final 20 episodes"],
            ["Maximum Accuracy", f"{max_accuracy:.4f}", "Best accuracy achieved"],
            ["Accuracy Std Dev", f"{accuracy_std:.4f}", "Training stability measure"],
            [
        ...
        plt.suptitle(
            "Training Metrics Comprehensive Analysis", fontsize=20, fontweight="bold", y=0.98
        )
        plt.tight_layout()
    ...
    def plot_evaluation_overview(
        self, reports_dir: Path, save_name: str = "evaluation_overview"
    ) -> None:
        ...
        try:
            summary_df = pd.read_csv(reports_dir / "evaluation_summary.csv")
            episode_details = pd.read_csv(reports_dir / "evaluation_episode_details.csv")
            predictions_df = pd.read_csv(reports_dir / "evaluation_detailed_predictions.csv")
        ...
        ax1.axhline(y=accuracy.mean(), color=self.colors["primary"], linestyle="--", alpha=0.7)
        ax1_twin.axhline(
        ...
        ax1_twin.set_ylabel("Average Confidence", color=self.colors["secondary"])
        ax1.set_title("Episode-wise Performance Trends", fontsize=14, fontweight="bold")
        ...
        ax2.axvline(
            0.8, color=self.colors["success"], linestyle=":", linewidth=2, label="Threshold: 0.8"
        )
        ...
        ax3.set_title("Prediction Time Distribution", fontsize=14, fontweight="bold")
...
Max: {pred_times.max():.2f}ms"""
        ...
        ax4.set_title("True vs Predicted Class Distribution", fontsize=14, fontweight="bold")
        ax4.set_xticks(x)
        ...
        predictions_df["confidence_bin"] = pd.cut(
            predictions_df["Confidence"], bins=confidence_bins, labels=bin_labels
        )

        bin_accuracy = predictions_df.groupby("confidence_bin")["Correct"].mean()
        bin_counts = predictions_df.groupby("confidence_bin").size()

        bars = ax5.bar(
            range(len(bin_accuracy)), bin_accuracy.values, alpha=0.7, color=self.colors["success"]
        )
        ...
        ax5.set_title("Accuracy by Confidence Level", fontsize=14, fontweight="bold")
        ...
        ax6.set_title("Confidence: Correct vs Incorrect", fontsize=14, fontweight="bold")
        ax6.legend()
        ax6.grid(True, alpha=0.3)
        ...
        metrics = ["overall_accuracy", "high_confidence_accuracy", "average_confidence"]
        if "macro_avg_precision" in summary_df.columns:
            metrics.extend(["macro_avg_precision", "macro_avg_recall", "macro_avg_f1_score"])
        ...
        ax8.plot(angles, values, "o-", linewidth=2, color=self.colors["primary"])
        ax8.fill(angles, values, alpha=0.25, color=self.colors["primary"])
        ax8.set_xticks(angles[:-1])
        ax8.set_xticklabels(labels[:-1], fontsize=10)
        ax8.set_ylim(0, 1)
        ax8.set_title("Performance Metrics Overview", fontsize=14, fontweight="bold", pad=20)
        ax8.grid(True)
...
"""
        ...
        ax10.set_title("Per-Class Performance Metrics", fontsize=14, fontweight="bold")
        ax10.set_xticks(x)
        ...
        plt.suptitle("Comprehensive Evaluation Analysis", fontsize=20, fontweight="bold", y=0.98)
        plt.tight_layout()
    ...
    def plot_class_analysis(self, reports_dir: Path, save_name: str = "class_analysis") -> None:
        ...
        cm = confusion_matrix(
            predictions_df["True_Label"], predictions_df["Predicted_Label"], labels=unique_labels
        )
        ...
        ax1.set_title("Detailed Confusion Matrix (Raw Counts)", fontsize=16, fontweight="bold")
        ...
        logger.debug(f"Classification report columns: {classification_report_df.columns.tolist()}")
        logger.debug(f"Classification report index: {classification_report_df.index.tolist()}")

        # Set the first column as index if it contains the class names
        if "Unnamed: 0" in classification_report_df.columns:
            classification_report_df = classification_report_df.set_index("Unnamed: 0")
                    ...
                    metrics_data.append(
                        {
                            ...
                            if "precision" in row_data
                            else 0.0,
                            "recall": float(row_data["recall"]) if "recall" in row_data else 0.0,
                            "f1-score": float(row_data["f1-score"])
                            if "f1-score" in row_data
                            else 0.0,
                            "support": float(row_data["support"])
                            if "support" in row_data
                            else 0.0,
                        }
                    )
                except (KeyError, ValueError) as e:
                    logger.warning(f"Could not extract metrics for class {label}: {e}")
        ...
        if not metrics_data:
            logger.warning("No metrics data found, calculating from predictions...")
        ...
        ax2.set_title("Per-Class Performance Metrics Comparison", fontsize=14, fontweight="bold")
        ax2.set_xticks(x)
        ...
        bars = ax3.bar(
            range(len(support_values)),
            support_values,
            color=self.class_colors[: len(support_values)],
            alpha=0.7,
        )
        ...
        ax3.set_title("Class Support Distribution", fontsize=14, fontweight="bold")
        ...
        scatter = ax4.scatter(
            metrics_df["recall"],
            metrics_df["precision"],
            s=metrics_df["support"] / max(metrics_df["support"]) * 500,
        ...
        bars = ax5.barh(
            ...
            sorted_metrics["f1-score"],
            color=self.class_colors[: len(sorted_metrics)],
            alpha=0.7,
        )
        ...
        for idx, (start_class, end_class) in enumerate(
            [(0, 5), (5, 10), (10, len(unique_labels))]
        ):
            ...
            if confidence_data:  # Only plot if we have data
                bp = ax.boxplot(confidence_data, labels=labels_for_plot, patch_artist=True)

                # Color the boxes
                for patch, color in zip(
                    bp["boxes"],
                    self.class_colors[start_class : start_class + len(confidence_data)],
                ):
                    patch.set_facecolor(color)
                    patch.set_alpha(0.7)

                ax.set_title(
                    f"Confidence Distribution - Classes {start_class}-{min(end_class - 1, len(unique_labels) - 1)}",
                    fontsize=14,
                    fontweight="bold",
                )
                ...
                ax.text(
                    0.5,
                    0.5,
                    f"No data for classes {start_class}-{end_class - 1}",
                    ha="center",
                    va="center",
                ...
                ax.set_title(
                    f"Confidence Distribution - Classes {start_class}-{end_class - 1}",
                    fontsize=14,
                    fontweight="bold",
                )

        plt.suptitle("Comprehensive Per-Class Analysis", fontsize=20, fontweight="bold", y=0.98)
        plt.tight_layout()
    ...
    def plot_error_analysis(self, reports_dir: Path, save_name: str = "error_analysis") -> None:
        ...
        if len(incorrect_df) == 0:
            logger.warning("⚠️ No incorrect predictions found - perfect accuracy!")
            return
        ...
        bars = ax1.bar(
            range(len(error_by_true)),
            error_by_true.values,
            color=self.colors["warning"],
            alpha=0.7,
        )
        ...
        for bar, val in zip(bars, error_by_true.values):
            ...
            ax1.text(
                bar.get_x() + bar.get_width() / 2.0,
                height + max(error_by_true.values) * 0.01,
                f"{int(val)}",
                ha="center",
                va="bottom",
                fontsize=10,
        ...
        ax2 = fig.add_subplot(gs[0, 1])
        error_by_pred = incorrect_df["Predicted_Label"].value_counts().sort_index()
        bars = ax2.bar(
            range(len(error_by_pred)),
            error_by_pred.values,
            color=self.colors["secondary"],
            alpha=0.7,
        )

        ax2.set_title("Errors by Predicted Class", fontsize=14, fontweight="bold")
        ...
        for bar, val in zip(bars, error_by_pred.values):
            ...
            ax2.text(
                bar.get_x() + bar.get_width() / 2.0,
                height + max(error_by_pred.values) * 0.01,
                f"{int(val)}",
                ha="center",
                va="bottom",
                fontsize=10,
        ...
        ax3.set_title("Confidence: Correct vs Incorrect", fontsize=14, fontweight="bold")
        ...
        misclass_pairs = incorrect_df.groupby(["True_Label", "Predicted_Label"]).size()
        top_misclass = misclass_pairs.nlargest(15)

        pair_labels = [f"{true}→{pred}" for (true, pred) in top_misclass.index]

        bars = ax4.bar(
            range(len(top_misclass)), top_misclass.values, color=self.colors["accent"], alpha=0.7
        )

        ax4.set_title("Top 15 Misclassification Patterns", fontsize=14, fontweight="bold")
        ...
        for bar, val in zip(bars, top_misclass.values):
            ...
            ax4.text(
                bar.get_x() + bar.get_width() / 2.0,
                height + max(top_misclass.values) * 0.01,
                f"{int(val)}",
                ha="center",
                va="bottom",
                fontsize=9,
        ...
        error_rate_by_bin = 1 - predictions_df.groupby("confidence_bin")["Correct"].mean()
        bin_counts = predictions_df.groupby("confidence_bin").size()

        bars = ax5.bar(
            range(len(error_rate_by_bin)),
            error_rate_by_bin.values,
            alpha=0.7,
            color=self.colors["warning"],
        )
        ...
        ax5.set_title("Error Rate by Confidence Bin", fontsize=14, fontweight="bold")
        ax5.set_xlabel("Confidence Bin")
        ...
        if len(high_conf_errors) > 0:
            high_conf_by_class = high_conf_errors["True_Label"].value_counts().sort_index()
            bars = ax6.bar(
                range(len(high_conf_by_class)),
                high_conf_by_class.values,
                color=self.colors["accent"],
                alpha=0.7,
            )

            ax6.set_title(
                f"High-Confidence Errors (≥0.8)\nTotal: {len(high_conf_errors)}",
                fontsize=14,
                fontweight="bold",
            )
            ...
            for bar, val in zip(bars, high_conf_by_class.values):
                ...
                ax6.text(
                    bar.get_x() + bar.get_width() / 2.0,
                    height + max(high_conf_by_class.values) * 0.01,
                    f"{int(val)}",
                    ha="center",
                    va="bottom",
                    fontsize=10,
            ...
            ax6.set_title("High-Confidence Errors (≥0.8)", fontsize=14, fontweight="bold")

        # 7. Confusion matrix of errors only
        ax7 = fig.add_subplot(gs[2, 2])

        unique_labels = sorted(predictions_df["True_Label"].unique())
        error_cm = confusion_matrix(
            incorrect_df["True_Label"], incorrect_df["Predicted_Label"], labels=unique_labels
        )

        # Only show non-zero entries
        mask = error_cm == 0
        im = ax7.imshow(error_cm, cmap="Reds", alpha=0.8)
        print(type(mask), type(im))
            ...
            for j in range(error_cm.shape[1]):
                if error_cm[i, j] > 0:
                    ax7.text(
                        ...
                        ha="center",
                        va="center",
                        color="white" if error_cm[i, j] > error_cm.max() / 2 else "black",
                        fontweight="bold",
                    )
        ...
        high_conf_errors_count = len(incorrect_df[incorrect_df["High_Confidence"]])
        high_conf_total = len(predictions_df[predictions_df["High_Confidence"]])
        high_conf_error_rate = (
            high_conf_errors_count / high_conf_total if high_conf_total > 0 else 0
        )
        ...
        for label in unique_labels:
            ...
            error_rates_by_class[label] = class_errors / class_total if class_total > 0 else 0
...
"""
        ...
        plt.suptitle("Comprehensive Error Analysis", fontsize=20, fontweight="bold", y=0.98)
        plt.tight_layout()
    ...
    def create_all_plots(
        self, reports_dir: Path = REPORTS_DIR, metrics_path: Optional[Path] = None
    ) -> None:
        ...
        missing_files = [f for f in required_files if not (reports_dir / f).exists()]

        if missing_files:
            logger.warning(f"⚠️ Missing evaluation files: {missing_files}")
            logger.info("💡 Run evaluation first: python -m rl_ids.modeling.evaluate")
        else:
...
def training_plots(
    metrics_path: Path = typer.Option(
        REPORTS_DIR / "training_metrics.csv", help="Path to training metrics CSV file"
    ),
    figures_dir: Path = typer.Option(FIGURES_DIR, help="Output directory for figures"),
...
def all_plots(
    reports_dir: Path = typer.Option(REPORTS_DIR, help="Reports directory"),
    figures_dir: Path = typer.Option(FIGURES_DIR, help="Output directory for figures"),
    metrics_path: Optional[Path] = typer.Option(None, help="Training metrics path"),
    dpi: int = typer.Option(300, help="Resolution for saved plots"),
):
