...
from rl_ids.config import FIGURES_DIR, PROCESSED_DATA_DIR, REPORTS_DIR, NORMALISED_DATA_FILE
...
def plot_confusion_matrix_heatmap(cm_path: Path, save_path: Path, class_names: Optional[list] = None):
...
def plot_data_distribution(
    data_path: Path = typer.Option(
        NORMALISED_DATA_FILE, help="Path to dataset CSV"),
...
def plot_classification_metrics(
    report_path: Path = typer.Option(REPORTS_DIR / "evaluation_classification_report.csv",
...
def plot_training_progress(
    metrics_path: Path = typer.Option(REPORTS_DIR / "training_metrics.csv",
...
def plot_enhanced_confusion_matrix(
    cm_path: Path = typer.Option(REPORTS_DIR / "evaluation_confusion_matrix.csv",
                                 help="Path to confusion matrix CSV"),
    output_path: Path = typer.Option(FIGURES_DIR / "enhanced_confusion_matrix.png",
...
def create_summary_dashboard(data_path: Path, reports_dir: Path, save_path: Path):
    ...
    try:
        ...
        if training_metrics.exists():
            ...
            ax2.text(0.5, 0.5, "Training metrics\nnot available",
        ...
        if eval_report.exists():
            ...
            ax3.text(0.5, 0.5, "Evaluation metrics\nnot available",
