...
from rl_ids.config import TEST_DATA_FILE
...
class IDSPredictionService:
    ...
    def __init__(self, model_path: Path):
        ...
        logger.info(
            f"Initializing prediction service with model: {model_path}")
    ...
    async def _load_feature_info(self) -> None:
        ...
        try:
            if TEST_DATA_FILE.exists():
                ...
                df_sample = pd.read_csv(TEST_DATA_FILE, nrows=1)
                self.feature_columns = [col for col in df_sample.columns
                                        if col not in ["Label", "Label_Original"]]
                logger.info(
                    f"Loaded {len(self.feature_columns)} feature columns")
            else:
                logger.warning(
                    "Normalized data file not found. Using default feature info.")
    ...
    async def predict(self, features: List[float]) -> Dict[str, Any]:
        ...
        try:
            ...
            features_tensor = torch.FloatTensor(
                features_array).unsqueeze(0).to(self.device)
