...
class IDSAPIClient:
    ...
    async def predict_batch(self, batch_features: List[List[float]]) -> List[Dict[str, Any]]:
        ...
        try:
            batch_payload = [{"features": features}
...
async def test_api_endpoints():
    ...
    try:
        ...
        logger.info(
            f"Prediction: {prediction['predicted_class']} (confidence: {prediction['confidence']:.3f})")
        ...
        for i, result in enumerate(batch_results):
            logger.info(
                f"  Batch {i+1}: {result['predicted_class']} ({result['confidence']:.3f})")
...
async def benchmark_api_performance(num_requests: int = 100):
    ...
    try:
        logger.info(
            f"🏁 Starting performance benchmark with {num_requests} requests...")
        ...
        batch_size = 10
        batch_data = [test_features[i:i+batch_size]
        ...
        logger.info(
            f"  Batch requests per second: {num_requests/batch_total_time:.2f}")
    ...
    try:
        ...
        for pattern_name, pattern_func in patterns.items():
            ...
            avg_confidence = sum(r['confidence']
                                 for r in pattern_results) / len(pattern_results)
            ...
            logger.info(
                f"  Attack rate: {results[pattern_name]['attack_rate']:.2%}")
            logger.info(
                f"  Avg confidence: {results[pattern_name]['avg_confidence']:.3f}")
...
async def main():
    ...
    try:
        ...
        logger.success(
            f"✅ All tests completed! Results saved to {results_file}")
