...
class IDSPredictionResponse(BaseModel):
    ...
    class_probabilities: List[float] = Field(..., description="Probability scores for each class")
    predicted_class: str = Field(..., description="Human-readable predicted class name")
    is_attack: bool = Field(..., description="Boolean indicating if traffic is malicious")
...
class ModelInfoResponse(BaseModel):
    ...
    training_episodes: Optional[int] = Field(None, description="Number of training episodes")
    model_size_mb: float = Field(..., description="Model size in megabytes")
    class_names: List[str] = Field(..., description="Names of the output classes")
