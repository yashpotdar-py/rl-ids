...
def main():
    ...
    parser.add_argument("--port", type=int,
                        default=settings.port, help="Port to bind to")
    parser.add_argument("--workers", type=int,
                        default=settings.workers, help="Number of worker processes")
    parser.add_argument("--reload", action="store_true",
                        help="Enable auto-reload for development")
    parser.add_argument(
        "--log-level", default=settings.log_level.lower(), help="Log level")
    ...
    if not settings.model_path.exists():
        ...
        logger.error(
            "Please ensure the model is trained and saved before starting the API")
        sys.exit(1)
